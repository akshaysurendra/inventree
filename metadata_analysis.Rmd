...
Our objectives are to understand (1) who and where tree-based data on India was being published from, (2) to understand geographic gaps in sampling effort (3) status of data availability across studies.
...
#2. only geocode if address is not in lookup table
...
#richness of corresponding authors
#diversity of corresponding authors
```
...
We have completed phase I of the INvenTree metadata analysis. Manual sorting resulted in `r lvl2_tot` publications that report tree inventories either using plot/transect methods (n=`r num_plot_trans`), checklists (n=`r num_check`) while `r num_unsure` were of uncertain methods.

Preliminary results show that publications spanned `r sum(journals)` journals, with the top 3 being `r names(journals)[1]` (n=`r journals[1]`), `r names(journals)[2]` (n=`r journals[2]`) and `r names(journals)[3]` (n=`r journals[3]`). The Shannon's diversity of journals was `r shannon.journal`, suggesting high dominance.

Corresponding authors were predominantly affiliated to institutions in `r max.cor.country` (n=`r cor.max` out of `r lvl2_tot`), followed by `r names(countries)[2]` (n=`r countries[2]`). Across the papers, there were `r sum(authors_geo$count)` points of authorship with the most number (n=`r country.freq[1]`) from `r names(country.freq)[1]`. Authors were second most frequently from `r names(country.freq)[2]` (n=`r country.freq[2]`).

In ongoing analyses, we are assessing the spread and accessibility of the data as well as gaps.
